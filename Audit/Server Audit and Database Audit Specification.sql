...
DECLARE @baseFolderPath NVARCHAR(255) = 'D:\AuditLogs\';
...
DECLARE @currentMonth NVARCHAR(7) = '2025-02-01'-- CONVERT(VARCHAR(7), GETDATE(), 23);  -- YYYYMM format for the current month
...
BEGIN
    ...
    BEGIN
		...
		---- Add action filtering (excluding 'AUSC')
  --      SET @sql = 'ALTER SERVER AUDIT [' + @auditName + '] WHERE action_id <> ''AUSC'';';
  --      EXEC sp_executesql @sql;
